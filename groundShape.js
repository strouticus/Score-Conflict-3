...
function GroundShape (partArray) {
		...
		for (var i = 0; i < this.partArray.length; i++)
		{
			if (this.partArray[i].x > checkX)
			{
				if (i > 0)
				{
					...
					return this.partArray[i - 1].y * (distToNext / dist) + this.partArray[i].y * (distToPrev / dist) - yAdjust;
				}
				else
				{
					return this.partArray[i].y - yAdjust;
				}
			}
		}
		return this.partArray[this.partArray.length - 1].y - yAdjust;
	}
	...
	this.GetAngleAtX = function (checkX) {
		return Math.atan2(this.GetSlopeAtX(checkX), 1);
	}
}